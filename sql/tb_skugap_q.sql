-- ══════════════════════════════════════════════════════════════════════════════
-- SKU Gap Recommendation — Talabat Platform (v3 — reads pre-materialized lookups)
--
...
--
-- Markets: TB_AE,TB_QA,TB_JO,TB_KW,TB_OM,TB_BH,TB_IQ,HF_EG
...
all_vendor_dims AS (
  ...
  WHERE global_entity_id IN ('TB_AE','TB_QA','TB_JO','TB_KW','TB_OM','TB_BH','TB_IQ','HF_EG')
...
all_vendor_product_sales AS (
  ...
  WHERE o.global_entity_id IN ('TB_AE','TB_QA','TB_JO','TB_KW','TB_OM','TB_BH','TB_IQ','HF_EG')
...
vendor_users AS (
  ...
  WHERE NOT COALESCE(u.is_deleted, FALSE)
    ...
    AND v.global_entity_id IN ('TB_AE','TB_QA','TB_JO','TB_KW','TB_OM','TB_BH','TB_IQ','HF_EG')
  GROUP BY ALL
),
