-- ══════════════════════════════════════════════════════════════════════════════
-- SKU Gap Recommendation — Hungerstation Platform (v3 — reads pre-materialized lookups)
--
...
all_vendor_dims AS (
  ...
  WHERE global_entity_id IN ('HS_SA')
...
all_vendor_product_sales AS (
  ...
  WHERE o.global_entity_id IN ('HS_SA')
...
vendor_users AS (
  ...
  WHERE NOT COALESCE(u.is_deleted, FALSE)
    ...
    AND v.global_entity_id IN ('HS_SA')
  GROUP BY ALL
),
