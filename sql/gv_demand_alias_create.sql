CREATE OR REPLACE TABLE `dh-darkstores-stg.local_shops_analytics.vendor_crm_comms_use_case_alias_table_GV` AS

-- ══════════════════════════════════════════════════════════════════════════════
-- Product Alias Mapping — Glovo Platform (22 markets, mixed languages)
...
-- ── CONFIG: Noise words per market (local language) ──────────────────────────
noise_words_local AS (
  SELECT * FROM UNNEST([
    -- Italian (removed: bottiglia, bottiglie, lattina)
    STRUCT('GV_IT' AS geid, r'(confezione|busta|barattolo|vasetto|cartone|bibita|bevanda|gassata|birra)' AS noise_regex),
    -- Spanish (removed: botella, lata, sixpack, doypack)
    STRUCT('GV_ES', r'(gaseosa|bebida|refresco|cerveza|cervezas|bandeja|bolsa|sobre|descartable|retornable)'),
    -- Portuguese (removed: garrafa, lata)
    STRUCT('GV_PT', r'(pacote|saco|frasco|caixa|embalagem|pack|refrigerante|bebida|cerveja)'),
    -- Ukrainian (Cyrillic — no \b; removed: пляшка, банка)
    STRUCT('GV_UA', r'(пакет|пакування|коробка|лоток|картон|напій|газований)'),
    -- Polish (removed: butelka, puszka)
    STRUCT('GV_PL', r'(opakowanie|torba|słoik|karton|napój|gazowany|piwo)'),
    -- Romanian (removed: sticlă, doză)
    STRUCT('GV_RO', r'(pachet|pungă|borcan|cutie|carton|băutură|răcoritoare|bere)'),
    -- Georgian (no \b; removed: ბოთლი, ქილა)
    STRUCT('GV_GE', r'(პაკეტი|ყუთი|კოლოფი|სასმელი|გაზიანი)'),
    -- Russian (for Kazakhstan — no \b; removed: бутылка, банка)
    STRUCT('GV_KZ', r'(пакет|упаковка|коробка|лоток|картон|напиток|газированный)'),
    -- Croatian (removed: boca, limenka)
    STRUCT('GV_HR', r'(pakiranje|vrećica|staklenka|kutija|piće|gazirani|pivo)'),
    -- Serbian (Cyrillic — no \b; removed: flaša, limenka)
    STRUCT('GV_RS', r'(pakovanje|kesa|tegla|kutija|piće|gazirani|pivo)'),
    -- Bulgarian (Cyrillic — no \b; removed: бутилка, кутия)
    STRUCT('GV_BG', r'(пакет|буркан|напитка|газирана)'),
    -- French (for Morocco, Ivory Coast; removed: bouteille, canette)
    STRUCT('GV_MA', r'(paquet|sachet|bocal|barquette|carton|flacon|pack|boisson)'),
    STRUCT('GV_CI', r'(paquet|sachet|bocal|barquette|carton|flacon|pack|boisson)'),
    -- Armenian (no \b; removed: շիշ, տուփ)
    STRUCT('GV_AM', r'(փաթեթ|տոպրակ|բաժակ|հատ|ըմպելիք|գազավորված)'),
    -- Bosnian (similar to Croatian/Serbian; removed: boca, limenka)
    STRUCT('GV_BA', r'(pakovanje|kesa|kutija|piće|gazirani)'),
    -- Montenegrin (similar to Serbian; removed: flaša, limenka)
    STRUCT('GV_ME', r'(pakovanje|kesa|kutija|piće|gazirani)'),
    -- Macedonian (Cyrillic — no \b)
    STRUCT('GV_MK', r'(шише|кутија|пакет|тегла|пиће|газиран)')
  ])
),
...
noise_removed AS (
  SELECT u.*,
    -- English noise removal (removed: bottle/bottles, can/cans)
    CASE WHEN u.eng_unit_normalized IS NOT NULL THEN
      REGEXP_REPLACE(
        u.eng_unit_normalized,
        r'\b(pack|packs|box|boxes|bag|bags|jar|jars|pouch|pouches|tin|tins|tray|trays|packet|packets|carton|cartons|sachet|sachets|tube|tubes|beer)\b',
        ''
      )
    END AS eng_clean,
    -- Local noise removal (market-specific regex from config)
